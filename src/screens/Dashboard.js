...
export default function Dashboard({ navigation }) {
  ...
  const { users: doctors, loading: doctorsLoading } = queryUsers({ type: "doctor" })
  ...
  return (
    <Container>
      ...
      <ScrollView className="rounded-[20px] flex-1 bg-white pt-5" refreshControl={<RefreshControl refreshing={appoinmentsLoading} onRefresh={get} />}>
        <View className="px-5">
          <Text className="text-[20px] font-bold">Asuncion's Doctors</Text>
        </View>
        {doctorsLoading ? <Shimmer style={{ height: 100, width: 200, borderRadius: 15, marginLeft: 15 }}/> : <FlatList
          data={doctors}
          horizontal
          renderItem={({ item }) => <TouchableOpacity style={styles.card}>
            <View className="flex-row">
              ...
              <View>
                ...
                <Text className="text-gray-500">{item.specialties}</Text>
              </View>
            </View>
            <View className="flex-row justify-center items-center mt-2">
              <Text className="font-bold">P {item.fee}</Text>
              <TouchableOpacity
                onPress={() => {
                  setState({ selectedDoctor: item })
                  navigation.navigate("Book")
                }}
                className="flex-row justify-center items-center gap-x-2 flex-1">
                <CalendarIcon />
                <Text className="text-[#FF7900] font-bold">Book Appointment</Text>
              </TouchableOpacity>
            </View>
        ...
        <View className="px-5 justify-between flex-row">
        ...
        {doctorsLoading ? <Shimmer style={{ height: 100, width: 200, borderRadius: 15, marginLeft: 15 }}/> : <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return <Card>
              <View className="flex-row">
                <Image source={{ uri: doctor.picture }} className="h-24 w-24 rounded-full mr-3" />
                <View>
                  <Text className="font-bold text-[18px]">{`${doctor.first_name} ${doctor.last_name}`}</Text>
